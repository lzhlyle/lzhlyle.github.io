...
- 使用 **左移**、**右移** 来表示棋子移动
    ...
    0000             0000
    ```
    - 曹操 `0110_0110_0000_0000_0000` 下移 `1` 行，得到 `0000_0011_0011_0000_0000`
        - 相当于拉伸成一维后右移 `4` 位
    ```java
    0110             0000
    0110             0110
    ...
    0000             0000
    ```
    - 小兵（当前棋盘在左下角） `0000_0000_0000_0000_1000` 向东北方移动（拐角移动算1步），得 `0000_0000_0000_0100_0000`
        - 相当于上移 `1` 行，再右移 `1` 列
        - 相当于左移 `3` 位
    ```java
    0000             0000             0000
...
- 通过位移后 `& 1` 判断是否到边
    ...
    ```
    - 五虎将 `1000_1000_0000_0000_0000` 在最上一行的最左边，右移 `15` 位后得 `0000_0000_0000_0001_0001` （别管当前对应什么位置，纯位移来判断原来是否是某行的左边），最低位是 `1` ，故 `return true;`
- 利用 `int` ，可高效排序，在压缩棋盘时，表示对同类型棋子，**总是** 无所谓先后（详见下文 *[棋局压缩](#棋局压缩)*）
    ```java
    // 每次压缩棋局前，先对同类型的棋子排序
    ...
    Arrays.sort(vArr);
    ```
- 利用只记录各棋子最低位的 `1` ，来实现 `long` 存储一个棋局（详见下文 *[棋局压缩](#棋局压缩)*）
    ```java
    int[] temp = new int[10];
    // ... 将棋局存入 temp 数组
    ...
    return res;
    ```
- 利用 `&` 作棋子重叠判断， `|=` 作占位累计（详见下文 *[可能的移动](#可能的移动)*）
    ```java
